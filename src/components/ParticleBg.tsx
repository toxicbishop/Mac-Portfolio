...
function getViewportMeta(W: number, H: number): ViewportMeta {
...
export default function ParticleBg() {
  ...
  const [mode, setMode] = useState<'moon' | 'sun'>(() => {
    const saved = localStorage.getItem(STORAGE_KEY)
    return saved === 'moon' ? 'moon' : 'sun'
  })

  useEffect(() => {
    const onSkyModeChange = (event: Event) => {
  ...
  useEffect(() => {
    ...
    const drawSunTheme = () => {
      ...
      const bloom = ctx.createRadialGradient(view.sunX, view.sunY, 0, view.sunX, view.sunY, Math.min(W, H) * (view.isMobile ? 0.28 : 0.34))
      ...
      const x = view.sunX, y = view.sunY, r = view.sunR
      ...
      const disc = ctx.createRadialGradient(x - r * 0.18, y - r * 0.18, r * 0.1, x, y, r)
    ...
    const drawMoonTheme = () => {
      ...
      bg.addColorStop(0, '#050816'); bg.addColorStop(0.45, '#08111f'); bg.addColorStop(1, '#02040b')
      ctx.fillStyle = bg; ctx.fillRect(0, 0, W, H)

      const leftFade = ctx.createLinearGradient(0, 0, view.leftDarkCutoff, 0)
      leftFade.addColorStop(0, 'rgba(0,0,0,0.72)'); leftFade.addColorStop(0.7, 'rgba(0,0,0,0.28)'); leftFade.addColorStop(1, 'rgba(0,0,0,0)')
      ctx.fillStyle = leftFade; ctx.fillRect(0, 0, W, H)

      const coolBloom = ctx.createRadialGradient(view.moonX, view.moonY, 0, view.moonX, view.moonY, Math.min(W, H) * (view.isMobile ? 0.22 : 0.28))
      coolBloom.addColorStop(0, 'rgba(95, 135, 255, 0.10)'); coolBloom.addColorStop(0.45, 'rgba(60, 95, 210, 0.06)'); coolBloom.addColorStop(1, 'rgba(255,255,255,0)')
      ctx.fillStyle = coolBloom; ctx.fillRect(0, 0, W, H)

      const x = view.moonX, y = view.moonY, r = view.moonR
      const glow = ctx.createRadialGradient(x, y, 0, x, y, r * 4)
      glow.addColorStop(0, 'rgba(255,255,245,0.12)'); glow.addColorStop(0.35, 'rgba(190,215,255,0.07)'); glow.addColorStop(1, 'rgba(255,255,255,0)')
      ctx.beginPath(); ctx.arc(x, y, r * 4, 0, Math.PI * 2); ctx.fillStyle = glow; ctx.fill()

      const body = ctx.createRadialGradient(x - r * 0.22, y - r * 0.22, r * 0.08, x, y, r)
      body.addColorStop(0, 'rgba(255,255,250,0.95)'); body.addColorStop(0.7, 'rgba(225,232,245,0.82)'); body.addColorStop(1, 'rgba(198,207,224,0.72)')
      ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.fillStyle = body; ctx.fill()

      ctx.beginPath(); ctx.arc(x + r * 0.34, y, r * 0.95, 0, Math.PI * 2)
    ...
    const drawShootingStars = (now: number) => {
      ...
      for (let i = shootingStars.length - 1; i >= 0; i--) {
        ...
        s.life -= s.decay
        if (s.life <= 0 || s.x > W + 320 || s.y > H + 320) { shootingStars.splice(i, 1); continue }
        const angle = Math.atan2(s.vy, s.vx)
        const tailLen = s.maxLen * s.life
        const tx = s.x - Math.cos(angle) * tailLen
        const ty = s.y - Math.sin(angle) * tailLen
        const layers = [
          { lw: (s.width + 5) * s.life, stops: [[0, 'rgba(255,255,255,0)'], [0.55, `rgba(150,180,255,${(s.life * 0.10).toFixed(3)})`], [1, `rgba(255,255,255,${(s.life * 0.28).toFixed(3)})`]] as [number, string][] },
          { lw: s.width * s.life, stops: [[0, 'rgba(255,255,255,0)'], [0.38, `rgba(235,242,255,${(s.life * 0.42).toFixed(3)})`], [1, `rgba(255,255,255,${(s.life * 0.78).toFixed(3)})`]] as [number, string][] },
        ]
        for (const layer of layers) {
          const grd = ctx.createLinearGradient(tx, ty, s.x, s.y)
          layer.stops.forEach(([pos, col]) => grd.addColorStop(pos as number, col as string))
          ctx.beginPath(); ctx.strokeStyle = grd; ctx.lineWidth = layer.lw; ctx.lineCap = 'round'
        ...
        head.addColorStop(0, `rgba(255,255,255,${s.life.toFixed(3)})`); head.addColorStop(0.35, `rgba(210,228,255,${(s.life * 0.62).toFixed(3)})`); head.addColorStop(1, 'rgba(160,185,255,0)')
        ctx.beginPath(); ctx.arc(s.x, s.y, hSize * 3, 0, Math.PI * 2); ctx.fillStyle = head; ctx.fill()
        s.x += s.vx; s.y += s.vy
      }
    }
    ...
    const frame = (now: number) => {
      ...
      if (mode === 'sun') { drawSunTheme() } else { drawMoonTheme(); drawStars(); drawShootingStars(now) }
